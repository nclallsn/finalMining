# Logistic Regression (multinomial) classifier training and evaluation.
...
logreg_params = default_params.copy()
if os.path.exists("best_logreg_params.json"):
    with open("best_logreg_params.json") as f:
        tuned_params = json.load(f)
    logreg_params.update(tuned_params)
    print(f"\nLoaded tuned hyperparameters from 'best_logreg_params.json': {tuned_params}")
else:
    print("\nNo 'best_logreg_params.json' found — using default hyperparameters. "
          "Run logreg_hyperparameter_tuning.py first to tune them.")
...
y_proba = logreg.predict_proba(X_test_reduced)

classes = logreg.named_steps["clf"].classes_

# Model size — serialize the trained pipeline (scaler + classifier) in memory
model_size_bytes = len(pickle.dumps(logreg))
...
importances = pd.Series(mean_abs_coef, index=X_train_reduced.columns)
...
plt.title(f"{MODEL_NAME} — Top 20 Feature Importances (mean |coefficient|)")
